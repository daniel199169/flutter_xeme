...
class Vimeo extends StatefulWidget {
  Vimeo({this.id, this.type, this.subOrder});
  final String id;
  final String type;
...
class _VimeoState extends State<Vimeo> {
  ...
  TextEditingController _vimeoURLController;
  ...
  void initState() {
    ...
    _vimeoURLController = TextEditingController(text: "");
  ...
  getData() async {
    ...
    VimeoModel _data = await BuildderManager.getVimeoData(
        uid, widget.id, widget.type, int.parse(widget.subOrder));
    setState(() {
      data = _data;
      _titleController = TextEditingController(text: data.title);
      _vimeoURLController = TextEditingController(text: data.vimeoURL);
  ...
  _saveBuilder() async {
    if (imageURL != null && imageURL != '') {
      VimeoModel vimeo = VimeoModel(
          ...
          tag: tag,
          reference: reference);
      BuildderManager.updateVimeo(vimeo, SessionManager.getUserId(), widget.id,
          widget.type, int.parse(widget.subOrder));
    }
  }
              ...
              child: Form(
                child: Column(
                  children: <Widget>[
                    ...
                    TextFormField(
                      controller: _vimeoURLController,
                      maxLines: 3,
                      keyboardType: TextInputType.emailAddress,
                      autofocus: false,
                      style: TextStyle(
                      ...
                      onChanged: (value) => vimeoURL = value.trim(),
                      onFieldSubmitted: (_vimeoURLController) {
                        _saveBuilder();
                      },
                    ),
                    SizedBox(
