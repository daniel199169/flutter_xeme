...
class Vimeo extends StatefulWidget {
  ...
  Vimeo({this.id, this.type, this.subOrder, this.pageId});
  @override
...
class _VimeoState extends State<Vimeo> {
  ...
  void addNewCollection(String collectionTitle) async {
    Collection collection = Collection(
        ...
        tag: '',
        reference: '');
    collection.image = vimeoList[int.parse(widget.subOrder)].image;
    collection.videoURL = vimeoList[int.parse(widget.subOrder)].vimeoURL;
    collection.sectionType = 'vimeo';
    collection.title = vimeoList[int.parse(widget.subOrder)].title;
    collection.description = vimeoList[int.parse(widget.subOrder)].description;
    collection.tag = vimeoList[int.parse(widget.subOrder)].tag;
    collection.reference = vimeoList[int.parse(widget.subOrder)].reference;
  ...
  getVimeo() async {
    setState(() {
      _videoPlayerController = new VideoPlayerController.network(
        vimeoList[int.parse(widget.subOrder)].vimeoURL,
      );

      // _videoPlayerController = new VideoPlayerController.asset('videos/zoom.mp4');
    });
  }
              ...
              child: Stack(
                children: <Widget>[
                  Container(
                    ...
                    child: vimeoList.length != 0
                        ? ClipRRect(
                            borderRadius: BorderRadius.circular(40.0),
                            child: CachedNetworkImage(
                              imageUrl:
                                  vimeoList[int.parse(widget.subOrder)].image,
                              width: MediaQuery.of(context).size.width,
                    ...
                    child: Align(
                      ...
                      child: ListView(
                        children: <Widget>[
                          ...
                          Text(
                            vimeoList[int.parse(widget.subOrder)].title,
                          ...
                          Text(
                            vimeoList[int.parse(widget.subOrder)].description,
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              fontSize: 12,
                        ...
                        onTap: () async {
                          if (SessionManager.getUserId() != '') {
                            ...
                            await AlertShowDialog(context,
                                ...
                                content:
                                    "Please login or signup to use the features!");
                          }
                        },
